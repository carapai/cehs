...
class AreaDataCard(DataCard):
    ...
    def _get_figure(self, data):
        ...
        for name, df in data.items():
            ...
            treemap_df = self.build_hierarchical_dataframe(df.reset_index(),
                                                           levels=df.index.names,
                                                           value_column=df.columns[0],
                                                           total_id=name)

            scale_max = round(df[df.columns[0]].max() / df[df.columns[0]].sum() * 100, 2)


            fig.add_trace(fig_type(
    ...
    def build_hierarchical_dataframe(self, df, levels, value_column, total_id='total'):
        ...
        total = pd.Series(dict(id=total_id, parent='',
                               value=df[value_column].sum(),
                               color=df[value_column].sum()))
        df_all_trees = df_all_trees.append(total, ignore_index=True)
        try:
            df_all_trees['color'] = df_all_trees['color'] / \
                df_all_trees['color'].max() * 100
