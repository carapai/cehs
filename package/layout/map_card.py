...
class MapDataCard(DataCard):
    ...
    def _get_figure(self, data: {str: pd.DataFrame} = None) -> go.Figure:
        ...
        # Update the map
        fig.update_layout(mapbox_style=self.mapbox_style,
    ...
    def __set_geo_data(self, gdf, tolerance=None, id='id'):
        ...
        # Set bounds
        bounds = gdf.bounds
        bounds = {'xmin': bounds.minx.min(),
                  'xmax': bounds.maxx.max(),
                  'ymin': bounds.miny.min(),
                  'ymax': bounds.maxy.max()}
        bounds['center'] = {'lon': np.mean([bounds.get('xmin'), bounds.get(
            'xmax')]), 'lat': np.mean([bounds.get('ymin'), bounds.get('ymax')])}
